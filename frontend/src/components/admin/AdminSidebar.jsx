import React from 'react';
import { NavLink, useNavigate, useLocation } from 'react-router-dom';
import { useClerk } from '@clerk/clerk-react';
import {
  Shield,
  LayoutDashboard,
  Users,
  UserCheck,
  Building2,
  Activity,
  Bot,
  Bell,
  UserCircle,
  HelpCircle,
  Settings,
  LogOut,
  X,
} from 'lucide-react';
import { adminProfileData } from '../../data/adminMockData';

const ADMIN_NAV_ITEMS = [
  { label: 'Dashboard',           icon: LayoutDashboard, href: '/admin/dashboard' },
  { label: 'User Management',     icon: Users,           href: '/admin/users' },
  { label: 'Doctor Management',   icon: UserCheck,       href: '/admin/doctors' },
  { label: 'Department Management', icon: Building2,     href: '/admin/departments' },
  { label: 'Workflow Monitoring', icon: Activity,        href: '/admin/workflow' },
  { label: 'AI Agent Activity',   icon: Bot,             href: '/admin/ai-agent', badge: 'Mock' },
  { label: 'Notifications',       icon: Bell,            href: '/admin/notifications', badge: 8 },
  { label: 'Profile',             icon: UserCircle,      href: '/admin/profile' },
];

const BOTTOM_ITEMS = [
  { label: 'Help & Support', icon: HelpCircle, href: '/admin/help' },
  { label: 'Settings',       icon: Settings,   href: '/admin/settings' },
];

export const AdminSidebar = ({ isOpen, onClose }) => {
  const { signOut } = useClerk();
  const navigate = useNavigate();
  const location = useLocation();

  const handleSignOut = async () => {
    sessionStorage.removeItem('mediflow_auth');
    localStorage.removeItem('mediflow_user_role');
    try {
      if (signOut) {
        await signOut();
      }
    } catch {
      // ignore in demo mode
    }
    navigate('/admin/login');
  };

  const isItemActive = (href) => {
    if (href === '/admin/dashboard') {
      return location.pathname === '/admin/dashboard';
    }
    return location.pathname.startsWith(href);
  };

  const sidebarContent = (
    <div className="flex flex-col h-full bg-white border-r border-[#E2E8F0] select-none">
      {/* ── Brand Logo & Portal Badge ── */}
      <div className="flex items-center justify-between px-5 py-5 border-b border-[#E2E8F0]">
        <div className="flex items-center gap-2.5">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-[#0F766E] to-[#115E59] text-white shadow-sm flex-shrink-0">
            <Shield className="h-5 w-5" />
          </div>
          <div className="leading-tight">
            <div className="text-sm font-extrabold tracking-wider text-[#0F172A] flex items-center gap-1.5">
              MEDIFLOW
              <span className="text-[10px] uppercase font-bold tracking-wider px-1.5 py-0.5 rounded bg-[#CCFBF1] text-[#0F766E] border border-[#0F766E]/20">
                ADMIN
              </span>
            </div>
            <div className="text-[11px] text-[#0F766E] font-semibold">
              Admin Portal
            </div>
          </div>
        </div>

        {/* Mobile close button */}
        <button
          onClick={onClose}
          className="lg:hidden flex h-8 w-8 items-center justify-center rounded-lg text-[#64748B] hover:bg-slate-100 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[#0F766E]"
          aria-label="Close navigation"
        >
          <X className="h-4 w-4" />
        </button>
      </div>

      {/* ── Main Navigation ── */}
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1" aria-label="Admin navigation">
        <div className="px-3 pb-1 text-[11px] font-semibold uppercase tracking-wider text-[#94A3B8]">
          System Administration
        </div>
        {ADMIN_NAV_ITEMS.map(({ label, icon: Icon, href, badge }) => {
          const isActive = isItemActive(href);
          return (
            <NavLink
              key={href}
              to={href}
              onClick={onClose}
              className={`group relative flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#0F766E] ${
                isActive
                  ? 'bg-[#CCFBF1]/60 text-[#0F766E] font-semibold shadow-xs'
                  : 'text-[#475569] hover:bg-slate-50 hover:text-[#0F172A]'
              }`}
            >
              {/* Active left accent indicator */}
              {isActive && (
                <span className="absolute left-0 top-1/2 -translate-y-1/2 h-5 w-1 rounded-r-full bg-[#0F766E]" />
              )}
              <Icon
                className={`h-4.5 w-4.5 flex-shrink-0 transition-colors ${
                  isActive ? 'text-[#0F766E]' : 'text-[#94A3B8] group-hover:text-[#475569]'
                }`}
              />
              <span className="flex-1 truncate">{label}</span>
              {badge !== undefined && (
                <span
                  className={`flex h-5 min-w-[20px] items-center justify-center rounded-full px-1.5 text-[10px] font-bold ${
                    typeof badge === 'string'
                      ? 'bg-purple-100 text-purple-700 font-semibold'
                      : isActive
                      ? 'bg-[#0F766E] text-white'
                      : 'bg-slate-100 text-[#475569] group-hover:bg-[#CCFBF1] group-hover:text-[#0F766E]'
                  }`}
                >
                  {badge}
                </span>
              )}
            </NavLink>
          );
        })}
      </nav>

      {/* ── Admin System Status Box ── */}
      <div className="px-3 pb-3">
        <div className="p-3 rounded-xl bg-slate-50 border border-[#E2E8F0] text-xs">
          <div className="flex items-center justify-between text-[#64748B] font-medium text-[11px] mb-1">
            <span>System Status</span>
            <span className="flex items-center gap-1 text-[#16A34A] font-semibold">
              <span className="h-1.5 w-1.5 rounded-full bg-[#16A34A] animate-pulse" />
              Healthy
            </span>
          </div>
          <div className="font-semibold text-[#0F172A] text-xs">{adminProfileData.hospital}</div>
          <div className="text-[11px] text-[#64748B] mt-0.5">Tier 1 Multi-tenant Instance</div>
        </div>
      </div>

      {/* ── Bottom Section & Profile ── */}
      <div className="px-3 py-3 border-t border-[#E2E8F0] space-y-0.5">
        {BOTTOM_ITEMS.map(({ label, icon: Icon, href }) => (
          <NavLink
            key={href}
            to={href}
            onClick={onClose}
            className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-[#64748B] hover:bg-slate-50 hover:text-[#0F172A] transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[#0F766E]"
          >
            <Icon className="h-4 w-4 flex-shrink-0 text-[#94A3B8]" />
            {label}
          </NavLink>
        ))}

        <button
          onClick={handleSignOut}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-[#DC2626] hover:bg-red-50 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-red-400 text-left cursor-pointer"
        >
          <LogOut className="h-4 w-4 flex-shrink-0" />
          Sign Out
        </button>

        {/* Admin Profile Area */}
        <div className="pt-2 mt-2 border-t border-[#E2E8F0] flex items-center gap-2.5 px-2 py-1.5">
          <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-[#0F766E] text-white font-bold text-xs shadow-xs">
            AD
          </div>
          <div className="min-w-0 leading-tight">
            <div className="text-xs font-bold text-[#0F172A] truncate">
              {adminProfileData.name}
            </div>
            <div className="text-[10px] text-[#0F766E] font-medium truncate">
              {adminProfileData.role}
            </div>
          </div>
        </div>
      </div>
    </div>
  );

  return (
    <>
      {/* Desktop: always visible fixed sidebar */}
      <aside className="hidden lg:block fixed top-0 left-0 h-screen w-64 z-30">
        {sidebarContent}
      </aside>

      {/* Mobile: slide-in drawer */}
      {isOpen && (
        <>
          <div
            className="lg:hidden fixed inset-0 z-40 bg-black/40 backdrop-blur-sm transition-opacity"
            onClick={onClose}
            aria-hidden="true"
          />
          <aside className="lg:hidden fixed top-0 left-0 h-screen w-72 z-50 shadow-2xl bg-white animate-in slide-in-from-left duration-200">
            {sidebarContent}
          </aside>
        </>
      )}
    </>
  );
};

export default AdminSidebar;
