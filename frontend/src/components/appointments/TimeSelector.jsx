...
export const TimeSelector = ({ selectedTime, onSelectTime }) => {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h4 className="text-sm font-bold text-[#17221B]">Select Consultation Time Slot</h4>
          ...
          {morningSlots.map((slot) => {
            ...
            return (
              <button
                ...
                className={`py-3 px-2 rounded-xl text-xs font-semibold border text-center transition-all duration-150 flex flex-col items-center justify-center gap-1 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#15803D] ${
                  !isAvailable
                    ? 'opacity-40 bg-slate-100 border-slate-200 text-slate-400 cursor-not-allowed line-through'
                    : isSelected
                    ? 'border-[#15803D] bg-[#15803D] text-white shadow-xs ring-1 ring-[#15803D] cursor-pointer'
