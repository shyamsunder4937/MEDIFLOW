...
export const AppointmentTable = ({
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#E2E8F0] shadow-xs overflow-hidden">
      {/* ── Table Header ── */}
      <div className="p-4 sm:p-5 border-b border-[#E2E8F0] flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-base sm:text-lg font-bold text-[#0F172A]">Scheduled Appointments</h2>
                    ...
                    <td className="py-3.5 px-4 text-right">
                      ...
                      >
                        ...
                        <button
                          type="button"
                          onClick={() => navigate(`/staff/appointments/${appt.id}`)}
                          className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-[#E2E8F0] bg-white text-xs font-semibold text-[#0F766E] hover:bg-[#CCFBF1]/40 transition-colors shadow-2xs cursor-pointer"
                          ...
                          {activeMenuId === appt.id && (
                            <div className="absolute right-0 top-full mt-1 w-44 bg-white rounded-xl shadow-lg border border-[#E2E8F0] py-1 z-30 animate-in fade-in-50 zoom-in-95">
                              <button
                                ...
                                className="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-[#475569] hover:bg-purple-50 hover:text-purple-700 text-left transition-colors"
                              >
                              ...
                              {appt.status !== 'Cancelled' && (
                                <button
                                  ...
                                  className="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-rose-600 hover:bg-rose-50 text-left transition-colors"
                                >
                ...
                <td colSpan={8} className="py-16 text-center">
                  <div className="flex flex-col items-center justify-center space-y-3 max-w-sm mx-auto">
                    ...
                    <div className="space-y-1">
                      <p className="text-sm font-bold text-[#0F172A]">No appointments found</p>
                      <p className="text-xs text-[#64748B]">
                        Try changing your search or filters to locate scheduled appointments.
                      </p>
                    </div>
                    {isFiltered && (
                      <button
      ...
      <div className="md:hidden divide-y divide-[#E2E8F0]">
        {appointments.length > 0 ? (
          appointments.map((appt) => (
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-mono font-bold text-xs text-[#0F766E]">{appt.id}</span>
                    <span className="text-xs font-semibold text-[#0F172A]">• {appt.time}</span>
                  </div>
                  <div className="font-bold text-sm text-[#0F172A] mt-0.5">{appt.patientName}</div>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs text-[#64748B] pt-1">
                ...
                <div className="flex items-center gap-1">
                  <Building2 className="h-3.5 w-3.5 text-[#0F766E] flex-shrink-0" />
                  <span className="truncate">{appt.department}</span>
                </div>
              </div>

              <div className="flex items-center justify-between text-[11px] text-[#64748B] bg-slate-50 p-2 rounded-xl">
                <div className="flex items-center gap-1">
              ...
              >
                {appt.status === 'Confirmed' && (
                  <button
                    type="button"
                    onClick={() => onCheckIn(appt)}
                    className="flex-1 inline-flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl text-xs font-bold bg-[#0F766E] text-white hover:bg-[#115E59] transition-colors"
                  >
                    <UserCheck className="h-3.5 w-3.5" />
                    <span>Check In</span>
                  </button>
                ...
                <button
                  type="button"
                  onClick={() => navigate(`/staff/appointments/${appt.id}`)}
                  className="flex-1 inline-flex items-center justify-center gap-1 py-2 px-3 rounded-xl text-xs font-semibold border border-[#E2E8F0] text-[#0F766E] bg-white hover:bg-[#CCFBF1]/30 transition-colors"
                >
                  <Eye className="h-3.5 w-3.5" />
                ...
                <button
                  ...
                  className="p-2 rounded-xl border border-slate-200 text-purple-700 bg-white hover:bg-purple-50 transition-colors"
                  title="Reschedule"
                >
          ...
          <div className="py-14 px-4 text-center space-y-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-slate-100 text-[#94A3B8] mx-auto">
              <SearchX className="h-6 w-6" />
            </div>
            <div className="space-y-1">
              <p className="text-sm font-bold text-[#0F172A]">No appointments found</p>
