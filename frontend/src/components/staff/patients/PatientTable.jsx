      ...
      <div className="p-4 sm:p-5 border-b border-[#E2E8F0] flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-base sm:text-lg font-bold text-[#0F172A]">Patients Directory</h2>
            <span className="text-xs font-bold text-[#0F766E] bg-[#CCFBF1] px-2.5 py-0.5 rounded-full border border-[#0F766E]/20">
              {patients.length} records
            </span>
          </div>
          <p className="text-xs text-[#64748B] mt-0.5">
            Click on any patient record to review complete demographic and admission history.
          </p>
        </div>
      </div>
                    ...
                    <td className="py-3.5 px-4 text-right">
                      ...
                      >
                        <button
                          type="button"
                          onClick={() => navigate(`/staff/patients/${patient.id}`)}
                          className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-[#E2E8F0] bg-white text-xs font-semibold text-[#0F766E] hover:bg-[#CCFBF1]/40 hover:border-[#0F766E]/40 transition-all shadow-2xs cursor-pointer"
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-slate-200 bg-white text-xs font-semibold text-[#475569] hover:bg-slate-100 hover:text-[#0F172A] transition-all shadow-2xs cursor-pointer"
                  ...
                  <div className="flex flex-col items-center justify-center space-y-3 max-w-sm mx-auto">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-[#64748B]">
                        Try changing your search or filters to locate registered records.
                      </p>
                    </div>
                    {isFiltered && (
                      <button
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#CCFBF1] text-[#0F766E] font-extrabold text-xs flex-shrink-0">
                      {initials}
                    </div>
                    <div>
                      <div className="font-bold text-sm text-[#0F172A]">{fullName}</div>
                      <div className="text-[11px] font-mono text-[#64748B]">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs text-[#64748B] pt-1">
                  <div className="flex items-center gap-1 truncate">
                    <Phone className="h-3.5 w-3.5 text-[#94A3B8] flex-shrink-0" />
                    <span>{patient.phone}</span>
                  </div>
                  <div className="flex items-center gap-1 truncate">
                ...
                >
                  <button
                    type="button"
                    onClick={() => navigate(`/staff/patients/${patient.id}`)}
                    className="flex-1 inline-flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl text-xs font-semibold border border-[#E2E8F0] text-[#0F766E] bg-white hover:bg-[#CCFBF1]/30 transition-colors"
                  >
                    <Eye className="h-3.5 w-3.5" />
                  ...
                  <button
                    type="button"
                    onClick={() => onEditPatient(patient)}
                    className="inline-flex items-center justify-center gap-1 py-2 px-3 rounded-xl text-xs font-semibold border border-slate-200 text-[#475569] bg-white hover:bg-slate-100 transition-colors"
                  >
                    <Edit3 className="h-3.5 w-3.5" />
                    <span>Edit</span>
                  </button>
